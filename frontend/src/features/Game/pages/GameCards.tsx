...
function GameCards({ token, userId, onEnergyConsumed, currentEnergy }: GameCardsProps) {
  ...
  return (
    <div className="w-full bg-gray-200 dark:bg-[#2c2c2c] p-6 rounded-xl shadow-md">
      {/* Filtre Panel */}
      <div className="flex justify-center gap-2 mb-6 p-2 bg-gray-100 dark:bg-[#1e1e1e] rounded-full border border-gray-300 dark:border-gray-500">  <button
        onClick={() => setLevelFilter("all")}
        className={`px-4 py-2 rounded-full text-sm font-bold transition ${levelFilter === "all"
            ? "bg-[#FDE68A] text-black"
            : "text-gray-600 dark:text-gray-400"
          }`}
      >
        {localize("gameUiTexts.allLevels")}
      </button>
        {[...Array(maxItemLevel)].map((_, i) => {
          ...
          return (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-bold transition ${levelFilter === level
                  ? "bg-[#FDE68A] text-black"
                  : "text-gray-600 dark:text-gray-400"
                }`}
            >
              {localize("gameUiTexts.levelAbbreviation")} {level}
            </button>
          );
        })}
      </div>
      ...
      {/* Eşya Kartları */}
      <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-6 rounded-xl p-6">  {itemLevels.length !== userItems.length ? (
        [...Array(6)].map((_, i) => (
          <div
            key={i}
            className="bg-gray-300 dark:bg-[#3a3a3a] aspect-[111/92] rounded-xl border border-gray-300 dark:border-gray-700 flex items-center justify-center text-gray-500"
          >
           {localize("gameUiTexts.loading")}
          </div>
        ))
      ) : filteredItems.length === 0 ? (
        <div className="col-span-full text-center text-gray-400 font-semibold text-sm py-8">
          {localize("gameUiTexts.noFilterItem")}
        </div>
      ) : (
        filteredItems.map((itemInstance) => {
          ...
          return (
            <div
              key={itemInstance.id}
              className={`relative aspect-[111/92] rounded-2xl overflow-hidden group bg-cover bg-center border border-gray-300 dark:border-gray-700 transition-all duration-300 ${levelGlowShadows[itemInstance.currentLevel] || ""
                }`}
            ...
            >
              ...
              {localize("gameUiTexts.level")} {itemInstance.currentLevel}
              </div>
              ...
              <div className="absolute bottom-0 left-0 right-0  p-3 space-y-1 z-10">
                ...
                <div className="flex flex-col gap-2 mt-2">
                  {isMaxLevel ? (
                    <button
                      disabled
                      className="w-full rounded-full h-8 flex items-center justify-center gap-2 bg-gray-300 dark:bg-gray-600 cursor-not-allowed shadow text-[#888] dark:text-[#aaa]"
                    >
                      <img src="/energyIcon.png" alt="Enerji" className="w-7 h-7" />   {localize("gameUiTexts.maxlevel")}
                    </button>
                  ) : showLevelUpButton ? (
                    <button
                    ...
                    >
                      ...
                      {localize("gameUiTexts.upgrade")}
                    </button>
                  ) : (
                    <>
                      <div className="relative w-full h-5 bg-gray-200 dark:bg-[#2b2b2b] rounded-full overflow-hidden border border-gray-300 dark:border-[#444]">
                        <div
                          className="absolute top-0 left-0 h-full bg-[#EE39A8] transition-all duration-500 ease-in-out"
                          style={{ width: `${progress}%` }}
                        />
                        <div className="absolute inset-0 flex items-center justify-center text-black dark:text-white text-[13px] font-bold">
                          %{progress}
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 mt-0">
                        <button
                          disabled={currentEnergy < selectedEnergy}
                        ...
                        >
                          <span className="flex items-center gap-1">
                            ...
                            -{selectedEnergy}   {localize("gameUiTexts.levelUp")}
                          </span>
                        </button>

                        <button
                          className="w-[20%] rounded-full font-bold h-8 flex items-center justify-center gap-1 shadow bg-gray-200 dark:bg-[#1e1e1e] text-black dark:text-white border border-gray-300 dark:border-[#444] hover:bg-gray-300 dark:hover:bg-[#2a2a2a] transition-colors"
              ...
              <div className="absolute inset-0 bg-black bg-opacity-10 dark:bg-opacity-30 group-hover:bg-opacity-20 dark:group-hover:bg-opacity-40 transition" />
            </div>
          );
        })
      )}
